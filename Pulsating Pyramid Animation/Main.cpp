...
{
    return Point2D{ WindowSizeX / 2 + (point.x * FL) / (FL + point.z) * 150, WindowSizeY / 2 + (point.y * FL) / (FL + point.z) * 150 };
}
...
int main(int argc, char* argv[])
{
    ...
    // Create window
    SDL_Window* window = SDL_CreateWindow("Pyramid Renderer No OOP", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1280, 720, 0);
    if (window == nullptr) {
